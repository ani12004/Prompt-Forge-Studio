...
import {
    ...
    Gamepad2, BarChart3, Mail, Eye, EyeOff, ArrowUp, Menu, X, ChevronsUpDown
} from "lucide-react"
...
export function DevDocsClient() {
    ...
    const toggle = useCallback((id: string) => {
        setCollapsed(prev => {
            ...
            const anyOpen = SECTIONS.some(s => !next[s.id])
    ...
    const toggleAll = useCallback(() => {
        ...
        SECTIONS.forEach(s => { next[s.id] = allExpanded })
        setCollapsed(next)
    ...
    const NavList = ({ inputRef, onItemClick }: { inputRef?: React.RefObject<HTMLInputElement | null>; onItemClick?: () => void }) => (
        <>
            <div className="p-3 border-b border-white/5">
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/5">
                    ...
                    />
                    <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-white/[0.06] border border-white/10 text-[10px] text-gray-500 font-mono">⌘K</kbd>
                </div>
            </div>
                    ...
                    {filtered.map(s => {
                        ...
                        return (
                            <li key={s.id}>
                                <a
                                    href={`#${s.id}`}
                                    onClick={e => { e.preventDefault(); document.getElementById(s.id)?.scrollIntoView({ behavior: "smooth" }); onItemClick?.() }}
                                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] transition-all duration-200 relative ${isActive
                                        ? "bg-brand-purple/10 text-white font-semibold"
                                        : "text-gray-500 hover:text-white hover:bg-white/[0.04]"
                                        }`}
                                >
                                    {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full bg-brand-purple transition-all duration-300" />}
                                    <span className={`transition-colors duration-200 ${isActive ? s.color : "text-gray-600"}`}>{s.icon}</span>
                                    {s.title}
                                </a>
                            </li>
                        )
    ...
    return (
        <div className="min-h-screen pb-32 pt-28 px-4 md:px-6 bg-[#050508]">
            ...
            <div className="fixed top-0 left-0 w-full h-[3px] z-[60] bg-transparent">
                <div
                    className="h-full bg-gradient-to-r from-brand-purple via-brand-indigo to-brand-violet transition-[width] duration-150 ease-out shadow-[0_0_10px_rgba(139,92,246,0.5)]"
            ...
            <button
                ...
                className="fixed top-20 left-4 z-50 lg:hidden p-2.5 rounded-xl bg-white/[0.05] border border-white/10 backdrop-blur-xl text-gray-400 hover:text-white hover:bg-white/[0.08] transition-all"
            ...
            {mobileOpen && (
                <div className="fixed inset-0 z-[55] lg:hidden">
                    ...
                    <aside className="absolute left-0 top-0 bottom-0 w-[280px] bg-[#0a0a0e]/95 backdrop-blur-xl border-r border-white/5 animate-slide-in-left">
                        <div className="flex items-center justify-between p-4 border-b border-white/5">
                            <span className="text-sm font-bold text-white">Navigation</span>
                            <button onClick={() => setMobileOpen(false)} className="p-1.5 rounded-lg hover:bg-white/5 text-gray-500 hover:text-white transition-colors">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        <NavList inputRef={mobileSearchRef} onItemClick={() => setMobileOpen(false)} />
                    </aside>
                </div>
            )}
            ...
            <div className="max-w-7xl mx-auto relative z-10 mb-12">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                    ...
                    <button
                        onClick={toggleAll}
                        className="self-start sm:self-auto inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/[0.03] border border-white/5 text-gray-500 hover:text-white hover:bg-white/[0.06] transition-all text-xs font-medium group"
                    >
                        <ChevronsUpDown className="w-3.5 h-3.5 group-hover:text-brand-purple transition-colors" />
                ...
                <main className="min-w-0 space-y-8">
                    {filtered.map(section => (
                        <section
                            ...
                            id={section.id}
                            data-doc-section
                            className="scroll-mt-28 rounded-2xl border border-white/5 bg-white/[0.015] backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-white/10"
                        >
                            {/* Section Header */}
                            <button
                            ...
                            >
                                <span className={`${section.color} transition-transform duration-300 ${collapsed[section.id] ? "" : "scale-110"}`}>{section.icon}</span>
                            ...
                            <AnimatedCollapse open={!collapsed[section.id]}>
            ...
            <button
                ...
                className={`fixed bottom-8 right-8 z-50 p-3 rounded-full bg-brand-purple/20 border border-brand-purple/30 text-brand-purple hover:bg-brand-purple/30 transition-all backdrop-blur-xl shadow-glow-sm ${showScrollTop ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
...
function AnimatedCollapse({ open, children }: { open: boolean; children: React.ReactNode }) {
    ...
    return (
        <div
            style={{ height: height !== undefined ? height : "auto", overflow: "hidden", transition: "height 0.35s cubic-bezier(0.4,0,0.2,1), opacity 0.3s ease", opacity: open ? 1 : 0 }}
            aria-hidden={!open}
        >
            <div ref={contentRef}>
                {children}
    ...
    return (
        <div className="overflow-x-auto my-4 rounded-xl border border-white/5">
            <table className="w-full text-sm">
                ...
                <tbody>
                    {rows.map((row, i) => (
                        <tr key={i} className={`border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors group/row ${i % 2 === 1 ? "bg-white/[0.008]" : ""}`}>
                            {row.map((cell, j) => (
                                <td key={j} className={`px-4 py-2.5 relative ${j === 0 ? "text-white font-mono text-[13px] font-medium" : "text-gray-500"}`}>
                                    {j === 0 && <span className="absolute left-0 top-0 bottom-0 w-[2px] bg-brand-purple/0 group-hover/row:bg-brand-purple/60 transition-colors duration-200" />}
                                    {cell}
                                </td>
                            ))}
                        </tr>
...
const ArchitectureSection = React.memo(function ArchitectureSection() {
    return (<>
        <P>The application follows a <strong className="text-white">three-layer architecture</strong>: Client → Server → Infrastructure.</P>
        <div className="my-4 rounded-xl border border-white/5 overflow-hidden bg-[#0a0a0d] group relative">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-brand-purple/10 to-brand-indigo/10 rounded-xl blur opacity-0 group-hover:opacity-100 transition duration-700" />
            <img
                src="/architecture-diagram.png"
                alt="PromptForge Studio System Architecture — Three-layer diagram showing Client, Server, and Infrastructure layers"
...
const EnvVarsSection = React.memo(function EnvVarsSection() {
    return (<>
...
const GettingStartedSection = React.memo(function GettingStartedSection() {
    return (<>
...
const DatabaseSection = React.memo(function DatabaseSection() {
    return (<>
